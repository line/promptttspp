...
Copied from https://github.com/nnsvs/nnsvs.
"""
...
class MDNLayer(nn.Module):
    ...
    def forward(self, minibatch):
        ...
        Returns:
            tuple:
                - torch.Tensor: Tensor of shape (B, T, G) or (B, T, G, D_out). Log of mixture weights. G is num_gaussians and D_out is out_dim.
                - torch.Tensor: Tensor of shape (B, T, G, D_out). The log of standard deviation of each Gaussian.
                - torch.Tensor: Tensor of shape (B, T, G, D_out). The mean of each Gaussian.
        """  # noqa: B950
        B = minibatch.shape[0]
        if self.dim_wise:
            # (B, T, G, D_out)
...
def mdn_loss(
    ...
    Args:
        ...
        mask: frame-level mask of shape (B, T, 1)
    ...
    if mask is not None:
        ...
        loss.masked_fill_(mask_expand, -float("inf"))
...
def mdn_sample_sigma_and_mu(log_pi, log_sigma, mu):
    dim_wise = len(log_pi.shape) == 4
    ...
    # (B, C, G)
    if dim_wise:
        pi = log_pi.exp().squeeze(1).transpose(1, 2)
    else:
        pi = log_pi.exp()

    # (B, C)
    selected_indices = torch.distributions.Categorical(probs=pi).sample()
    # (B, C, G)
    one_hot = F.one_hot(selected_indices, pi.shape[-1])
    ...
    # of the Gaussians whose weight coefficient(log_pi) is the largest.
    #  (B, T, G, d_out) -> (B, T, d_out)
    mu = torch.sum(mu * one_hot, dim=2)
    sigma = torch.exp(torch.sum(log_sigma * one_hot, dim=2))
